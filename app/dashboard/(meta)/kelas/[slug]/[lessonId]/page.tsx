...
export default async function LessonPage({ params }: { params: Promise<{ slug: string; lessonId: string }> }) {
  ...
  if (!canAccess) {
    return (
      <div className="p-6 max-w-2xl mx-auto">
        <Link href={`/dashboard/kelas/${c.slug}`} className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-[#0866FF] mb-6">
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      <Link href={`/dashboard/kelas/${c.slug}`} className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-[#0866FF] mb-4">
        <ArrowLeft className="w-4 h-4" /> {c.title}
      </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          {/* Player video: embed YouTube bila ada, kalau tidak placeholder */}
          {embedUrl ? (
          ...
          <div className="flex items-center justify-between gap-3">
            {prev ? (
              <Link href={`/dashboard/kelas/${c.slug}/${prev.id}`} className="flex items-center gap-1.5 text-sm font-semibold text-gray-600 hover:text-[#0866FF] transition-colors">
                <ChevronLeft className="w-4 h-4" /> Sebelumnya
              </Link>
            ) : <span />}
            {next ? (
              nextAccessible ? (
                <Link href={`/dashboard/kelas/${c.slug}/${next.id}`} className="flex items-center gap-1.5 bg-[#0866FF] hover:bg-[#0757d4] text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors">
