...
import { Printer, ArrowLeft, Video, Check, ShieldCheck } from "lucide-react";

const NAVY1 = "#0f1729";
const NAVY2 = "#15233f";
const INK = "#1c2b33";

export default function WebinarCertificate({
  name,
  webinarTitle,
  speaker,
  score,
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="no-print flex items-center justify-between">
        <Link href={`/dashboard/webinar/${webinarId}`} className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-[#0866FF]">
          <ArrowLeft className="w-4 h-4" /> Kembali ke Webinar
        </Link>
        <button
          onClick={() => window.print()}
          className="inline-flex items-center gap-2 text-white text-sm font-bold px-4 py-2.5 rounded-xl transition-colors"
      ...
      <div id="cert-area">
        <div className="relative bg-white rounded-2xl overflow-hidden shadow-xl border border-gray-100">
          ...
          <div className="relative px-10 py-8 text-center overflow-hidden" style={{ background: headerBg }}>
            ...
            <div className="relative flex items-center justify-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-white/55 text-[10px] font-semibold uppercase tracking-[0.25em]">Webinar & Workshop Digital Marketing</p>
              </div>
            </div>
          </div>
          ...
          <div className="absolute left-0 right-0 bottom-0 top-24 flex items-center justify-center pointer-events-none select-none overflow-hidden">
            <Video className="w-72 h-72" style={{ color: accent, opacity: 0.04 }} strokeWidth={1} />
          </div>

          {/* Body */}
          <div className="relative px-8 py-9 md:px-16 md:py-11 text-center">
            <h2 className="text-2xl md:text-3xl font-extrabold tracking-[0.14em]" style={{ color: INK }}>
            ...
            <p className="mt-5 text-sm text-gray-500 max-w-xl mx-auto leading-relaxed">
              telah mengikuti sesi webinar dan dinyatakan <b style={{ color: INK }}>LULUS</b> ujian sertifikasi webinar
            </p>
            <p className="mt-2 text-lg md:text-2xl font-bold" style={{ color: accent }}>{webinarTitle}</p>

            {/* Pill info */}
            <div className="mt-3.5 flex items-center justify-center gap-2 flex-wrap">
              <span className="text-[11px] font-semibold px-3 py-1 rounded-full flex items-center gap-1" style={{ backgroundColor: `${accent}12`, color: accent }}>
                Webinar Online
              </span>
              <span className="text-[11px] font-semibold px-3 py-1 rounded-full flex items-center gap-1" style={{ backgroundColor: `${accent}12`, color: accent }}>
                Pemateri: {speaker}
              </span>
              <span className="text-[11px] font-bold px-3 py-1 rounded-full text-white" style={{ backgroundColor: accent }}>
                Predikat: {predikat}
              </span>
            </div>

            {/* Kompetensi dari webinar */}
            <div className="mt-7 mx-auto max-w-2xl text-left rounded-2xl p-5" style={{ backgroundColor: `${accent}08`, border: `1px solid ${accent}20` }}>
              ...
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2.5">
                {[
                  "Pemahaman materi webinar",
                  "Kemampuan analisis digital marketing",
                  "Implementasi strategi Meta Ads",
                  "Evaluasi performa iklan berbasis data",
                ].map((k, i) => (
            ...
            {/* Footer: tanggal · segel · tanda tangan */}
